...
const setting = computed(() => {
  ...
  setting.forEach((item, index) => {
    ...
    // 计算label折线起始点的位置
    item.lineStart = getPoint(state.R, startAngel + (endAngel - startAngel) / 2, center);
    // 线条label折线起点x值小于原点x值，在左侧，否则在右侧
    item.isLeft = item.lineStart[0] < center[0];
    // 根据线条起点左右，设置结束点, (0: svg最左边， width: 最右边)
    item.lineEnd = [(item.isLeft ? center[0] - state.R - 100 : center[0] +  state.R + 100), item.lineStart[1] - 60 / (index + 1)];
...
function getLabelLine(v) {
  return `${v.lineStart.join(' ')}, ${v.lineEnd[0] + (v.isLeft ? 50 : -50)}  ${v.lineEnd[1]}, ${v.lineEnd.join(' ')}`
}
